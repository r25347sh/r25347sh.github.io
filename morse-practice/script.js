 ...
 * 和文・欧文・数字・記号対応
 *
 * キーイング: ITU 1:3:7 比率対応 + 文字間・単語間の自動間隔検出
 * スペースキー感度向上
 */
...
function speedToUnitMs(speed, isWabun = true) {
  if (isWabun) return Math.max(20, Math.round(6500 / speed));
  const wpm = speed / 5;
...
// ========== Improved Keying (ITU 1:3:7 + auto interval) ==========
let isKeyDown = false;
let keyDownTime = 0;
let currentSymbols = [];
let decodedText = "";
...
function endKey() {
  ...
  const unit = getCurrentUnitMs();
  // 短点判定をやや敏感に (2.0単位未満)
  const symbol = duration < unit * 2.0 ? "." : "-";
...
function setupKeying() {
  ...
  document.addEventListener("keydown", e => {
    ...
    e.preventDefault();
    if (!spaceDown) { spaceDown = true; startKey(); }
  }, { passive: false });
  document.addEventListener("keyup", e => {
    if (e.code !== "Space") return;
    if (document.activeElement && (document.activeElement.tagName === "INPUT" || document.activeElement.tagName === "TEXTAREA" || document.activeElement.isContentEditable)) return;
    e.preventDefault();
    if (spaceDown) { spaceDown = false; endKey(); }
  }, { passive: false });
  document.addEventListener("keydown", e => {
    if (e.code === "Space" && !(document.activeElement && (document.activeElement.tagName === "INPUT" || document.activeElement.tagName === "TEXTAREA"))) e.preventDefault();
  }, { passive: false });
